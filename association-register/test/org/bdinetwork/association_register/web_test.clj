...
(deftest test-web
  (testing "with client-id"
    (let [{:keys [status body]}
          (handler (-> (request/request :get "/parties/EU.EORI.CLIENT")
                       (assoc :client-id "EU.EORI.CLIENT")))]
      (is (= http-status/ok status))
      (is (string/starts-with? body "{\"party_token\":\""))))

  (testing "without client-id"
    (let [{:keys [status body]}
          (handler (request/request :get "/parties/EU.EORI.CLIENT"))]
      (is (= http-status/unauthorized status))
      (is (string/blank? body)))))
